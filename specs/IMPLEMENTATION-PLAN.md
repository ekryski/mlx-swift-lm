...
| 6 | **017 — prefix KV cache** | Snapshot target KV state at request end keyed on stable prefix; hydrate on next request with the same prefix. All five phases consolidated: Phase 1 (`PrefixKVCache` + `PrefixKey` + LRU + stats), Phase 1B (per-class `serialise()` / `hydrate(from:)` + `generate()` wiring with stream wrapper + defence-in-depth `quantisationKindMismatch` guard), Phase 2 (`LastAssistantOpenerPolicy` for Qwen / Gemma 4 / GPT-OSS), Phase 3 (hybrid GDN+attention via spec 020 state-replay — `SSMStateCache` snapshots), Phase 4 (opt-in disk persistence at `~/.cache/mlx-swift-lm/prefix/`, off by default). Cross-session sharing via `BlockAllocator.retain()` tracked in [#133](https://github.com/ekryski/mlx-swift-lm/issues/133), separate future spec. | **Measured 2026-05-12 (M1 Max 64GB, --kv none, realistic 4-turn chat — actual model replies captured & fed back):** **Qwen3.5-35B-A3B ~4.3× TTFT** (2186ms→499ms) with prefill rate climbing 17→562 tok/s across turns (**33× prefill** at turn 4). Qwen3.5-0.8B ~1.0× (cold path already <100ms — no room to win). GPT-OSS-20B: no benefit (window=128 sliding-window wrap, documented limitation). | ✅ Ready-for-merge 2026-05-12 ([PR #144](https://github.com/ekryski/mlx-swift-lm/pull/144)) |
