...
|---|---|---|---|---|
| 5 | **020 phases 1–3 — state-replay rollback (GDN)** | Generalised the GDN rollback primitive so any speculative decoder works on hybrid SSM/Mamba models. `StateReplayCache` protocol + `SSMStateCache` conformance + native `gated_delta_step_record` / `state_replay` Metal kernels + n-gram iterator wiring. Mamba (Nemotron-H, Jamba) cleanly opts out via per-instance `canStateReplay = false` until the kernel variant lands (see row 6b). | Unlocked n-gram speculative decoding on Qwen 3.5 / 3.6 GDN hybrids. **Measured on M1 Max 64GB:** Qwen3.5-35B-A3B-4bit D=12 adapt+strict **1.64× baseline** (92% accept), D=4 **1.37×** (83% accept). Mamba families fall back to vanilla `TokenIterator` until kernel variant lands. | ✅ Shipped 2026-05-11 ([PR #143](https://github.com/ekryski/mlx-swift-lm/pull/143) + cross-repo chain [mlx#26](https://github.com/ekryski/mlx/pull/26) / [mlx-c#14](https://github.com/ekryski/mlx-c/pull/14) / [mlx-swift#25](https://github.com/ekryski/mlx-swift/pull/25)) |
| 6 | **017 — prefix KV cache** | Snapshot target KV state at request end keyed on stable prefix; hydrate on next request with the same prefix. All five phases consolidated: Phase 1 (`PrefixKVCache` + `PrefixKey` + LRU + stats), Phase 1B (per-class `serialise()` / `hydrate(from:)` + `generate()` wiring with stream wrapper + defence-in-depth `quantisationKindMismatch` guard), Phase 2 (`LastAssistantOpenerPolicy` for Qwen / Gemma 4 / GPT-OSS), Phase 3 (hybrid GDN+attention via spec 020 state-replay — `SSMStateCache` snapshots), Phase 4 (opt-in disk persistence at `~/.cache/mlx-swift-lm/prefix/`, off by default), Phase 5 (Option A: snapshot post-prefill, before decode wraps anything — fixes #185 / #196 / #197 + spec 017 §5 SSM stale + restores warm-turn hits on Gemma 4 26B/31B at `prompt_len == sliding_window`). **Opt-in for v1** (`prefixCacheEnabled = false` default). When the caller opts in, `prefixCachePolicy` auto-resolves via `AssistantOpener.detect(forModelID:)` → `LastAssistantOpenerPolicy` for Qwen / Gemma / GPT-OSS families (Identity fallback), and `prefixCacheModelID` auto-resolves from `ModelContext.configuration.name`. Cross-session sharing via `BlockAllocator.retain()` tracked in [#133](https://github.com/ekryski/mlx-swift-lm/issues/133). Compressed-domain snapshots (~4× smaller) move to spec 039 (row 6a). | **Measured 2026-05-12 (M1 Max 64GB, `--method summarization --kv turbo4v2 --cache-prefix`, Option A post-prefill snapshot):** Qwen3.5-0.8B 32K cold 18.9s → warm 322 ms (**59×** TTFT). Qwen3.5-0.8B 1K cold 459ms → warm 222ms (**2.1×**). Gemma 4 26B-A4B / 31B coherent multi-turn with `hits=1 prefillTokensSaved=34`. GPT-OSS-20B still no benefit on the Path A fallback (window=128 wrap during prefill — `serialiseStandard` correctly refuses; spec 039 doesn't fix this either since it's a wrap issue, not a compression issue). | ✅ Shipped 2026-05-12 ([PR #144](https://github.com/ekryski/mlx-swift-lm/pull/144) + close-out [PR #198](https://github.com/ekryski/mlx-swift-lm/pull/198)). Closes [#185](https://github.com/ekryski/mlx-swift-lm/issues/185). [#196](https://github.com/ekryski/mlx-swift-lm/issues/196) and [#197](https://github.com/ekryski/mlx-swift-lm/issues/197) remain **OPEN** as known limitations — Option A's post-prefill snapshot didn't actually fix them (Gemma 4 26B/31B inserts but never hits with turbo4v2; compressed-mode TurboQuant snapshot is still refused at `serialiseTurbo`). Documented in [`generate-parameters.md`](../documentation/generate-parameters.md#known-limitations); both gate re-enabling default-on prefix cache. |
| 6a | **039 — compressed-domain prefix KV cache** | Snapshot KV in TurboQuant compressed form (~4× smaller bytes via `valueBits=2` packed indices + norms) and hydrate back into compressed buffers without going through FP16. Reuses spec 017 phase 5's post-prefill hook, `TurboQuantizedKVCache.fusedEncodeDispatch` for the snapshot-time batch encode, and `compressedAttention(L>1)` for the warm suffix prefill (the L>1 path already exists; today's dispatcher in `AttentionUtils.attentionWithCacheUpdate` just routes it to the raw path). Lossless (compressed-bytes round-trip — no compress / dequant cycle on the warm path) and zero new Metal kernels — purely a re-route of existing primitives. Bumps `PrefixKey.formatVersion` 2 → 3 (L2 disk-cache migration). | **~3.4× smaller snapshot bytes** at the same `--kv turbo4v2` precision: Qwen3.5-35B-A3B 32K snapshot drops ~1.65 GB → ~485 MB; Gemma 4 26B-A4B 4K drops ~440 MB → ~130 MB. At the default 8 GB L1 budget this is 3.4× more cached prefixes resident → fewer LRU evictions on extended multi-session workloads. **Warm-turn TTFT:** estimated 50–200 ms faster (no FP16-load + first-decode-compression). **Decode tok/s:** unchanged (compressed-domain decode is the same post-hydrate). | 📋 Spec written 2026-05-12 ([`039-compressed-prefix-kv-cache.md`](039-compressed-prefix-kv-cache.md)). Not started. Phase 1 + 2 are the load-bearing pieces; phase 3 is just a format-version bump. |
| 6b | **040 — Mamba / Mamba 2 state-replay rollback** | Native `ssm_step_record` + `ssm_replay` Metal kernel pair, parallel to spec 020's shipped `gated_delta_step_record` / `state_replay` chain (spec 040 picks option 1 of spec 020's three follow-up options — preferred over sequential fallback or `ssmAttn` augmentation). Same 4-repo PR chain shape: mlx kernel + C++ Primitive → mlx-c bridge → mlx-swift Swift wrapper → mlx-swift-lm cache wiring. Flips `canStateReplay` from `false` → `true` on `NemotronH.swift` SSM cache slots; `canRollbackPromptCache` lights up; n-gram speculative + prefix-cache (spec 017) + spec 039 compressed prefix + future spec 015 DFlash all start working on Mamba families with no extra change. | Unblocks n-gram + prefix-cache + DFlash for **Nemotron Cascade 2 (Mamba)** and **Granite-MoE-Hybrid / FalconH1 (Mamba 2)** — all three share the `ssmUpdate` Mamba 2 selective-state formulation that matches the shipped kernel. Measured 2026-05-13 on Nemotron-30B-A3B + `--ngram 3`: coherent multi-turn output at 21.4% draft-acceptance rate. Round-trip unit tests (`testMambaStateReplayRoundTrip`, `testSSMStateCacheMambaRollback`) cover the kernel pair. | ✅ **Shipped 2026-05-13** for NemotronH / GraniteMoeHybrid / FalconH1 via cross-repo chain (mlx kernel + Primitive → mlx-c bridge → mlx-swift wrapper → mlx-swift-lm `SSMStateCache.recordMambaConvPadded(_:)` + `rollback(acceptedPrefix:)` conv-state slicing). **Open caveat: Jamba not yet integrated** — its `ssmStep` uses a 2D `A_log` shape that doesn't match the kernel's `[H]` ALog signature; needs either kernel-side shape generalisation or Swift-side reformulation in `Jamba.swift`. Tracked as a follow-up; not blocking on near-term bench targets. |
| 6c | **GPT-OSS-20B attention-sinks on TurboQuant** | Make `--kv turbo*` coherent on attention-sinks models (GPT-OSS family). The original pass2-sinks-fold approach (mlx#16 / mlx-c#8 / mlx-swift#18 / mlx-swift-lm#99) was abandoned: kernel math was right on paper but the cross-block softmax fusion produced incoherent output. Replaced 2026-05-13 by three orthogonal pieces: (1) a new single-pass **`turbo_flash_sdpa_v`** Metal kernel that does inline dequant + online softmax + sinks fold in one dispatch (sidesteps the pass1/pass2 graph-fusion bug); (2) **DC-bias correction** in the MSE codec (per-vector mean subtraction before WHT — recovers the structured DC offset that K/V from `RMSNorm → Linear(bias=True)` carry, which the zero-mean Lloyd-Max codebook can't represent — 9-22% MSE reduction on this distribution); (3) a **hybrid sliding-FP16 layer policy** for GPT-OSS-20B (sliding-window layers cap at 128 tokens, FP16 is ~1.5 MB across all sliding layers vs the full-attention layers' 8K-token compressed caches — full-attention layers stay on `TurboQuantizedKVCache` with `useBias: true` by default). Plumbing also routes sinks through the dequant-SDPA-with-sinks A path for the L=1 decode case. | GPT-OSS-20B coherent end-to-end on `--kv turbo*` at 1k + 8k summarization (verified with stochastic sampling — both contexts produce proper Harmony channel preamble + content). KV cache: 46 MB at 8K vs ~200 MB FP16 (~4× savings preserved). Decode tok/s: ~50 tok/s at 8K vs ~57 tok/s pre-fix (5-11% bias overhead — fusable into the dequant kernel as a follow-up). | ✅ **Shipped 2026-05-13** via cross-repo chain:<br>• mlx: [`5c86d7ec`](https://github.com/ekryski/mlx/commit/5c86d7ec) `turbo_flash_sdpa_v` kernel + sinks function constant, [`14728215`](https://github.com/ekryski/mlx/commit/14728215) bit-contiguous unpacking fix, [`78011998`](https://github.com/ekryski/mlx/commit/78011998) `ensure_contiguous` on kernel inputs (kv-head-1+ silent-zero fix), [`283df4cf`](https://github.com/ekryski/mlx/commit/283df4cf) `copy.h` include path<br>• mlx-swift: [`1886126`](https://github.com/ekryski/mlx-swift/commit/1886126) `MLXFast.turboFlashSDPAv` wrapper + submodule bumps<br>• mlx-swift-lm (PR [#215](https://github.com/ekryski/mlx-swift-lm/pull/215) chain): [`cf516e3`](https://github.com/ekryski/mlx-swift-lm/commit/cf516e3) routes sinks through `turbo_flash_sdpa_v`, [`db74f37`](https://github.com/ekryski/mlx-swift-lm/commit/db74f37) routes sinks through dequant-SDPA-with-sinks A path, [`afa8cc9`](https://github.com/ekryski/mlx-swift-lm/commit/afa8cc9) DC-bias correction + hybrid sliding-FP16 policy, [`93f191b`](https://github.com/ekryski/mlx-swift-lm/commit/93f191b) defaults `useBias: true` on GPT-OSS turbo caches<br>• Tests: `testTurboFlashSDPAvSinksSlidingWindow`, `testTurboFlashSDPAvNoSinksMatchesTurboFlashAttention`, `testTurboQuantCompressedAttentionSinksMatchesReference`, `testDualScaleMSEReduction` (MSE-comparison probe that drove the bias-over-dual-scale decision)<br>• Documentation: [`documentation/kv-cache.md`](../documentation/kv-cache.md) (new `useBias` behaviour-knob row + `TURBO_BIAS` env var), [`documentation/models.md`](../documentation/models.md) (GPT-OSS-20B known-gap row updated)<br>• **Superseded PRs (closed)**: mlx#16, mlx-c#8, mlx-swift#18, mlx-swift-lm#99 — all closed when the new approach landed. Umbrella [#130](https://github.com/ekryski/mlx-swift-lm/issues/130) closed 2026-05-14 with a comment pointing to the actual fix.<br>• **Defence-in-depth follow-up** ([mlx-swift#34](https://github.com/ekryski/mlx-swift/pull/34), 2026-05-14): added `tools/audit-mirror.sh` + `audit-mirror.yml` CI gate to catch the kernel-mirror body-drift class of bug surfaced by the [`14728215`](https://github.com/ekryski/mlx/commit/14728215) bit-contiguous fix landing in mlx-core but not in the SPM-compatible mirror at `Source/Cmlx/mlx-generated/metal/`. The audit walks every `.metal`/`.h` in the mirror, normalises submodule include paths, diffs bodies — fast Linux job (~10s, no Metal toolchain), runs on every PR / push to alpha. Closes the gap that `make doctor`'s signature-only check left open.<br>• Open follow-up: dual-scale codec (empirical 1-8% MSE gain — too small to ship given kernel cost) and model-aware codebook are documented in spec 041 phase 1.1 follow-up notes but not pursued. |
| 6d | **043 — TurboFlash decode-time kernel uplift** | Close the speed gap between TurboFlash (the new unconditional A path on TurboQuant caches, shipped at [`85afa9b`](https://github.com/ekryski/mlx-swift-lm/commit/85afa9b)) and dequant-SDPA via four Metal-kernel optimisations, in bench-evidence order of leverage. **Phase 1 — Per-simdgroup bit-unpack reuse:** cache the unpacked K block in threadgroup memory once per tile so 31 of every 32 SIMD lanes read from cache instead of re-running bit-extract + codebook lookup + norm multiply. Directly attacks the linear-in-context per-decode overhead that the 8k regression highlights. **Phase 2 — bf16 V accumulator (with fp32 softmax m/l):** drop the per-tile value-aggregation tensor from `float` to `half`, keep softmax running-max / denominator in `float` for stability. Documented 5-8% lift in [#158](https://github.com/ekryski/mlx-swift-lm/issues/158). **Phase 3 — headDim-aware tile autotune:** current `flashBlockSize` adapts on tokenCount but not on `headDim` — Qwen 0.8B/2B (headDim=64) are dramatically over-tiled. Add a `(headDim) → (NR0, blockSize)` static table picked at kernel-dispatch time. **Phase 4 — Bias-aware kernel (GPT-OSS-20B on A path):** add `key_bias` / `val_bias` + per-codec `rotated_ones` constants to the TurboFlash kernels (`turbo_flash_sdpa_v` primarily, plus the `turbo_flash_p1_*` family for symmetry). Inside reconstruction, add `bias[t] * rotated_ones[d]` to the rotated K/V. Unlocks GPT-OSS-20B (and any future sinks-using bias-correcting models) on the A path — today bias=true forces B regardless of `TURBO_DEQUANT_SDPA`. Can run in parallel with Phases 1-3 (additive kernel changes, no template restructure). **NOT a resurrection of closed PRs** (mlx#16, mlx-c#8, mlx-swift#18, mlx-swift-lm#99) which tried to fix the original pass2 sinks-fold bug — that's already solved by `turbo_flash_sdpa_v`'s single-pass design; Phase 4 is new bias-aware kernel work. Each phase its own PR; cross-repo chain (mlx kernel → mlx-swift submodule bump → mlx-swift-lm dispatch on Phases 3+4 only). Composes with [spec 042](042-metal-kernel-simd-audit.md) (broader audit); 043 is the focused subset that today's [A-path](../benchmarks/m1-max-64gb-2026-05-13.A-path.md) / [B-path](../benchmarks/m1-max-64gb-2026-05-13.B-path.md) bench identified as highest-leverage. | **2026-05-14 A-vs-B sweep across 156 cells (13 models × 6 KV × {1k, 8k}) at HEAD `f4d13bc`:** mean turbo* decode regression A vs B is -22% across all 13 models (8k worst: -56% on Qwen 0.8B turbo8v4 8k; -53% on Gemma 4 31B turbo8v4 8k). Acceptance gate after all four phases: TurboFlash decode tok/s ≥ 90% of dequant-SDPA on every cell; per-model worst delta improves from -56% to ≤ -15%; mean turbo* regression drops from -22% to ≤ -8%; GPT-OSS-20B coherent on the A path with `TURBO_DEQUANT_SDPA=0`. Memory invariant unchanged (same packed K/V storage). ~3.5-4.5 weeks single-engineer end-to-end depending on Phase 4 parallelism. | 📋 Spec written 2026-05-14 ([`043-turboflash-kernel-uplift.md`](043-turboflash-kernel-uplift.md)); Phase 4 added 2026-05-14. Not started. Branch `ek/spec-043-turboflash-kernel-uplift` carries spec only; per-phase implementation branches follow once Phase 1 begins. |
...
| 25 | **041 — Flash-style quantized SDPA** | Drop-in Flash-tiled fused kernel for the `quantizedScaledDotProductAttention` path. Replaces the discrete `quantizedMM → softmax → quantizedMM` chain (which materialises a `[B, H, L, T]` score tensor — ~1 GiB per layer at L=1024, T=32k) with an online-softmax tile loop that dequants K/V inside the kernel. Four-repo PR chain (`mlx` Metal kernel + C++ Primitive → `mlx-c` ABI → `mlx-swift` wrapper → `mlx-swift-lm` callsite swap in `KVCache.quantizedScaledDotProductAttention`). Phased: (1) `bits ∈ {4, 8}`, `groupSize=64`, causal + sliding-window, GQA/MQA, w/ matrix-engine MMA + threadgroup-memory codebook + INT8 score accumulator + pre-folded V codec rotation ([#118](https://github.com/ekryski/mlx-swift-lm/issues/118)), (2) `bits ∈ {2, 3, 6}` + mxfp4 dequant prologue, (3) TurboQuant compressed-domain L>1 reuse (post-[spec 039](039-compressed-prefix-kv-cache.md)), (4) attention-sinks fold (composes with Tier 1 row 6c), (5) KV-sharing reader path — shared readers in Gemma 4 E2B / E4B + Gemma 3n consume the donor's affine `(wq, scales, biases)` tuple directly via the new kernel, retiring the `StandardKVCache` fallback that `makeAttentionCache` currently routes through whenever a layer is flagged `architecturalSlidingWindow` or `forceRawKV` (closes [#202](https://github.com/ekryski/mlx-swift-lm/issues/202)). **Scope is the affine path** — `--kv none` and TurboQuant L=1 / L>1 raw-key already use Flash via `MLXFast.scaledDotProductAttention` or `TurboFlash`; only the affine + post-spec-039 turbo compressed-L>1 path materialises scores today. | **Peak GPU drop on long-context affine prefill.** Closes the residual ~1–2 GiB gap between affine peak and no-quant peak at ctx=32k (Qwen3.5-0.8B 4.03 GiB → ~2.0 GiB; Qwen3.5-9B 8.81 GiB → ~7.0 GiB). Prefill tok/s within 5% of discrete-path baseline (no perf regression). Decode unchanged (single-tile loop = same compute, less workspace). Phase 5 additionally restores affine compression on Gemma 4 sliding-window + KV-sharing layers (currently fall back to `StandardKVCache` for coherence). After Tier 1–3 stabilise. | 📋 Spec written 2026-05-12 ([`041-flash-quantized-sdpa.md`](041-flash-quantized-sdpa.md)) + Phase 5 + kernel-level optimisations added 2026-05-12. Issue [#202](https://github.com/ekryski/mlx-swift-lm/issues/202) tracks the Phase 5 fallback. Not started. |
| 26 | **042 — Metal kernel SIMD-optimisation audit** | One-time perf-uplift sweep across hand-rolled Metal kernels in the `ekryski/mlx` fork: convert TurboFlash + affine flash + `turbo_dequant_rotated` + `mse_*` to use `simdgroup_matrix_multiply_accumulate` MMAs (instead of `float4` scalar SIMD), hoist codebooks / rotation matrices into threadgroup memory, audit vec-load coalescing, profile for register spills, emit `(headDim, bits, groupSize)`-specialised instantiations (same template fan-out Apple's `scaled_dot_product_attention.metal` uses), and run the numeric-format audit ([#162](https://github.com/ekryski/mlx-swift-lm/issues/162) bf16→fp16 host-side; [#158](https://github.com/ekryski/mlx-swift-lm/issues/158) fp32 → fp16 V accumulator inside the kernels) under one rollup rather than as separate one-off PRs. Phased: (1a) TurboFlash MMA conversion + fp16 V accumulator (highest leverage — closes the A-path vs B-path gap that today's principled-default rework accepted), **(1b) Affine flash kernel MMA conversion** — subsumes [spec 041 Phase 1.2](041-flash-quantized-sdpa.md), reuses 1a's template + tile geometry (only dequant prologue differs: affine `(packed × scale) + bias` vs turbo `codebook[idx] × norm`), flips `MLX_AFFINE_SDPA` default from `.flash` (dequant-then-SDPA) to `.kernel` (fused) on decode as well as prefill — (2) `turbo_dequant_rotated` MMA, (3) fallback `mse_score` / `mse_weighted_sum`, (4) encode-path profiling. Compatibility: scalar fallback on M1 (limited SME). Composes with spec 041 (architecture) and spec 043 (TurboFlash-specific kernel uplift) — 042 is the cross-kernel SIMD tuning rollup. **Subsumes [#158](https://github.com/ekryski/mlx-swift-lm/issues/158) + [#162](https://github.com/ekryski/mlx-swift-lm/issues/162) + spec 041 Phase 1.2.** | **Close the ~8× compute-density gap between hand-rolled turbo / affine kernels and Apple's first-party SDPA.** Concrete: TurboFlash decode on Gemma 4 31B turbo4v2 32k currently runs 5.6 tok/s vs Apple's dequant-first path 9.6 tok/s — pure SIMD tuning gap, same workload. Affine flash kernel (shipped in spec 041 Phase 1.1) shows 47% prefill / 18% decode regression vs dequant-then-MLXFastSDPA on Qwen 0.8B 8k — same SIMD gap. Acceptance: TurboFlash matches dequant-SDPA across the 156-cell A-vs-B matrix; `MLX_AFFINE_SDPA` default flips to `.kernel`. After spec 041 Phase 1 lands (✅) or in parallel. | 📋 Spec written 2026-05-12 ([`042-metal-kernel-simd-audit.md`](042-metal-kernel-simd-audit.md)), Phase 1b added 2026-05-14. Not started. |
...
| [#158](https://github.com/ekryski/mlx-swift-lm/issues/158) | Float16 V accumulator in TurboFlash pass2 kernel (separate from the bf16 output dtype that landed in `74024a24`) — ~5–10% attention kernel improvement. **Subsumed by spec 042** (Tier 4 row 26) on 2026-05-12. | S | 🟢 OPEN — tracked under spec 042 |
...
| Issue | Topic | Size | Status |
|---|---|---|---|
| [#162](https://github.com/ekryski/mlx-swift-lm/issues/162) | fp16 vs bf16 runtime dtype audit + conversion (Tier 4 row 21). **Subsumed by spec 042** (Tier 4 row 26) on 2026-05-12. | M | 🟢 OPEN — tracked under spec 042 |
...
- ✅ **Tier 1 row #6c — GPT-OSS-20B attention-sinks on TurboQuant** shipped 2026-05-13 via the [PR #215](https://github.com/ekryski/mlx-swift-lm/pull/215) chain. Replaced the abandoned pass2-sinks-fold approach (mlx#16 / mlx-c#8 / mlx-swift#18 / mlx-swift-lm#99 — superseded) with a single-pass `turbo_flash_sdpa_v` Metal kernel + DC-bias correction + hybrid sliding-FP16 policy. GPT-OSS-20B coherent on `--kv turbo*` at 1k + 8k; KV cache ~4× smaller than FP16 at 8K. Closes umbrella [#130](https://github.com/ekryski/mlx-swift-lm/issues/130).
