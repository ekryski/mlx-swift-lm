...
| 6b | **040 — Mamba / Mamba 2 state-replay rollback** | Native `ssm_step_record` + `ssm_replay` Metal kernel pair, parallel to spec 020's shipped `gated_delta_step_record` / `state_replay` chain (spec 040 picks option 1 of spec 020's three follow-up options — preferred over sequential fallback or `ssmAttn` augmentation). Same 4-repo PR chain shape: mlx kernel + C++ Primitive → mlx-c bridge → mlx-swift Swift wrapper → mlx-swift-lm cache wiring. Flips `canStateReplay` from `false` → `true` on `NemotronH.swift` + `Jamba.swift` SSM cache slots; `canRollbackPromptCache` lights up; n-gram speculative + prefix-cache (spec 017) + spec 039 compressed prefix + future spec 015 DFlash all start working on Mamba families with no extra change. | Unblocks n-gram + prefix-cache + DFlash for **Nemotron Cascade 2 (Mamba)** and **Jamba / Granite-MoE-Hybrid / FalconH1 (Mamba 2)**. Estimated n-gram lift parallel to spec 020 GDN measurements: Nemotron Cascade 2-30B-A3B 4bit at D=12 adapt+strict should land in the **1.4–1.7× baseline** range (similar accept-rate profile to Qwen3.5-35B-A3B at the same model class). Brings the Mamba families to feature parity with GDN. ~1500 LOC across the 4-repo chain (similar scope to spec 020 phases 1–3). | 📋 Spec written 2026-05-12 ([`040-mamba-state-replay.md`](040-mamba-state-replay.md)). Not started. **Promoted to Tier 1 (2026-05-12)** to land alongside row 6a — both close out the "model-family feature parity" gap. |
...
| 6d | **043 — TurboFlash decode-time kernel uplift** | Close the speed gap between TurboFlash (the new unconditional A path on TurboQuant caches, shipped at [`85afa9b`](https://github.com/ekryski/mlx-swift-lm/commit/85afa9b)) and dequant-SDPA via three Metal-kernel optimisations, in bench-evidence order of leverage. **Phase 1 — Per-simdgroup bit-unpack reuse:** cache the unpacked K block in threadgroup memory once per tile so 31 of every 32 SIMD lanes read from cache instead of re-running bit-extract + codebook lookup + norm multiply. Directly attacks the linear-in-context per-decode overhead that the 8k regression highlights. **Phase 2 — bf16 V accumulator (with fp32 softmax m/l):** drop the per-tile value-aggregation tensor from `float` to `half`, keep softmax running-max / denominator in `float` for stability. Documented 5-8% lift in [#158](https://github.com/ekryski/mlx-swift-lm/issues/158). **Phase 3 — headDim-aware tile autotune:** current `flashBlockSize` adapts on tokenCount but not on `headDim` — Qwen 0.8B/2B (headDim=64) are dramatically over-tiled. Add a `(headDim) → (NR0, blockSize)` static table picked at kernel-dispatch time. Each phase its own PR; cross-repo chain (mlx kernel → mlx-swift submodule bump → mlx-swift-lm dispatch on Phase 3 only). Composes with [spec 042](042-metal-kernel-simd-audit.md) (broader audit); 043 is the focused subset that today's [A-path](../benchmarks/m1-max-64gb-2026-05-13.A-path.md) / [B-path](../benchmarks/m1-max-64gb-2026-05-13.B-path.md) bench identified as highest-leverage. | **2026-05-14 A-vs-B sweep across 156 cells (13 models × 6 KV × {1k, 8k}) at HEAD `f4d13bc`:** mean turbo* decode regression A vs B is -22% across all 13 models (8k worst: -56% on Qwen 0.8B turbo8v4 8k; -53% on Gemma 4 31B turbo8v4 8k). Acceptance gate after all three phases: TurboFlash decode tok/s ≥ 90% of dequant-SDPA on every cell; per-model worst delta improves from -56% to ≤ -15%; mean turbo* regression drops from -22% to ≤ -8%. Memory invariant unchanged (same packed K/V storage). ~3 weeks single-engineer end-to-end. | 📋 Spec written 2026-05-14 ([`043-turboflash-kernel-uplift.md`](043-turboflash-kernel-uplift.md)). Not started. Branch `ek/spec-043-turboflash-kernel-uplift` carries spec only; per-phase implementation branches follow once Phase 1 begins. |
...
| 26 | **042 — Metal kernel SIMD-optimisation audit** | One-time perf-uplift sweep across hand-rolled Metal kernels in the `ekryski/mlx` fork: convert TurboFlash + `turbo_dequant_rotated` + `mse_*` to use `simdgroup_matrix_multiply_accumulate` MMAs (instead of `float4` scalar SIMD), hoist codebooks / rotation matrices into threadgroup memory, audit vec-load coalescing, profile for register spills, emit `(headDim, bits, groupSize)`-specialised instantiations (same template fan-out Apple's `scaled_dot_product_attention.metal` uses), and run the numeric-format audit ([#162](https://github.com/ekryski/mlx-swift-lm/issues/162) bf16→fp16 host-side; [#158](https://github.com/ekryski/mlx-swift-lm/issues/158) fp32 → fp16 V accumulator inside the kernels) under one rollup rather than as separate one-off PRs. Phased: (1) TurboFlash MMA conversion + fp16 V accumulator (highest leverage — flips dequant-first vs flash routing decision), (2) `turbo_dequant_rotated` MMA, (3) fallback `mse_score` / `mse_weighted_sum`, (4) encode-path profiling. Compatibility: scalar fallback on M1 (limited SME). Composes with spec 041 — 041 is the architecture, 042 is the SIMD tuning. **Subsumes [#158](https://github.com/ekryski/mlx-swift-lm/issues/158) + [#162](https://github.com/ekryski/mlx-swift-lm/issues/162)** (formerly standalone Tier 4 line items). | **Close the ~8× compute-density gap between hand-rolled turbo kernels and Apple's first-party SDPA.** Concrete: TurboFlash decode on Gemma 4 31B turbo4v2 32k currently runs 5.6 tok/s vs Apple's dequant-first path 9.6 tok/s — pure SIMD tuning gap, same workload. Acceptance: TurboFlash matches or beats dequant-first across the smoke matrix; retires the `headDim < 256` workaround that the immediate PR ships. After spec 041 Phase 1 lands or in parallel. | 📋 Spec written 2026-05-12 ([`042-metal-kernel-simd-audit.md`](042-metal-kernel-simd-audit.md)). Not started. |
