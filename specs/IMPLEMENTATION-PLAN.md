...
| 10a | **031 — vertical-slash sparse prefill** | A-shape + vertical-slash patterns via existing SDPA primitives — no new kernel. Calibration sidecar per model selects head patterns; prefill runs masked SDPA over the sparse pattern. Composes multiplicatively with 032. | **4–7× prefill at 128K** | Long-context prefill, RAG, doc QA |
| 10b | **032 — speculative prefill** | Drafter-scored span selection during prefill (PFlash-style); drafter shared with spec 015 DFlash. No new kernel. Stacks on top of 031. | **8–12× TTFT at 128K**; 30–80× when stacked with 031 | Long-context TTFT |
| 10c | **034 — decode-side K-side top-k (Quest / RetrievalAttention)** | Per-layer per-step K-side top-k selection on a paged or contiguous cache. Three selectors: (1) block-mean LSH, (2) H2O heavy-hitter retention, (3) recency+sinks baseline. V1 ships on `StandardKVCache` with dense SDPA on gathered slots; V2 uses spec 033's fused block-sparse kernel; phase 7 wires the TurboQuant Path B fast path. **Closes the K-side decode gap left by TurboQuant Path B's sparse-V kernel** (which only attacks V-side waste). Spec 035 (below, Tier 4) is a refinement that swaps in the original Quest K_max/K_min selector. | **5–8× long-context decode** (at 32–128K, k≈2048); composes multiplicatively with 031/032 | Long-context decode, retrieval workloads |
| 10d | **036 — DuoAttention retrieval/streaming head split** | Per-head classification (calibration pass on synthetic NIAH) tags each attention head as **retrieval** (full KV) or **streaming** (sink+window). Streaming heads reuse PR #186's `TurboQuantizedKVCache(maxSize:)` directly; retrieval heads keep full cache. Two-cache-per-layer dispatch (Option A) lands first; optional ragged-shape Metal kernel (Option C) in phase 5. Composes orthogonally with 034/035: Quest applies only to the retrieval-head cache. | **1.5–2.2× decode + 1.7–2.6× memory at long context** ([paper](https://arxiv.org/abs/2410.10819): MHA 2.18× / 2.55×, GQA 1.50× / 1.67×); calibration is one-time per model. Stacks multiplicatively with 034. | Long-context chat, RAG, doc QA — every workload where #186 windowed eviction already helps |
...
| 24 | **035 — Quest K_max/K_min selector (refinement of 034)** | Drop-in replacement for spec 034's "block-mean LSH" selector with the original Quest paper's elementwise K_max / K_min upper bound. Tighter score bound → better NIAH retention at smaller k → smaller K-side compute budget for same retrieval fidelity. Costs 2× page metadata (~6% of K-cache vs ~3%). Implementation reuses 034 phases 1–3 (block-summary infrastructure, selector dispatch, NIAH harness); only the per-block scorer differs. **Ships only if 034 V1's measured NIAH curve doesn't already hit the target operating point.** | Marginal lift over 034 — **+10–25% extra k-budget headroom** at the same NIAH retention, or matched k-budget with cleaner failure mode on multi-hop reasoning. Bench-driven decision. | Long-context reasoning, multi-hop retrieval where 034 V1 falls short. After 034 V1 ships and NIAH curves are measured. |
| 25 | **037 — TEAL activation thresholding on `FusedGateUpMLP`** | Training-free magnitude thresholding inserted before `down_proj` in `FusedGateUpMLP`: prune entries of `silu(gate) * up` below a per-tensor calibrated threshold τ_l, giving ~50% MLP activation sparsity. Lossless within ~1% PPL at 40–50% sparsity per the paper. Two phases: (1) threshold-and-mask hook + per-model calibration recipe in `scripts/teal_calibrate.py`; (2) **block-sparse Metal kernel** for `(masked_mlp_act, down_proj) → out` — the load-bearing piece, ~3–4 weeks. Composes orthogonally with TurboQuant (activation sparsity is independent of weight quantization). **Best on dense models** — MoE already has sparsity at the expert level. | Paper: **1.53× @ 40% sparsity, 1.8× @ 50%** on Llama/Mistral 7B–70B (H100 wallclock). Honest M-series estimate: **1.2–1.4× decode lift** for the MLP fraction of total time. The MLP share of decode time is lower on Apple Silicon at long context (attention reads dominate past 32K), so the headline gain is smaller than published. | Dense models (Qwen 3.5 dense tiers, Gemma 4 31B). Post-Tier-3, in parallel with 036 phase 5 if Metal-kernel work concentrates. |
...
- ✅ Specs 031–034 (sparse-attention prefill/decode roadmap) landed on alpha via PR #188 (2026-05-09); now slotted into Tier 2 rows 10a–10c + Tier 3 row 11a.
- ✅ Specs 035 / 036 / 037 (Quest K_max/K_min refinement, DuoAttention, TEAL) landed in PR #189 (2026-05-09); slotted as Tier 4 row 24 / Tier 2 row 10d / Tier 4 row 25 respectively.
...
- 🔜 **Spec 036 (DuoAttention)** is the highest-leverage post-#186 spec — calibration framework + dual-cache dispatch slots directly onto the windowed turbo cache from PR #186; ~3–4 weeks for V1.
- 🔜 **Spec 034 V1** unlocks K-side decode wins at long context; depends on `StandardKVCache` only (no #127 dependency for V1); ~4–6 weeks.
