...
| 6c | **GPT-OSS-20B attention-sinks on TurboQuant path B** | Four-repo cross-cutting PR chain to make the compressed-domain decode path (`-compact` suffix, `useCompressedAttention=true`) coherent on attention-sinks models. Path B + non-sinks works coherently; path B + sinks is wired but disabled because output is incoherent despite kernel math being right on paper. The cross-repo chain has been OPEN since before spec 020 shipped; main outstanding work is identifying the divergence between path A (coherent) and path B (incoherent) on GPT-OSS-20B with the same sinks tensor (sliding-window + graph-fuser already ruled out). | Brings GPT-OSS-20B to feature parity with Qwen 3.5 / Gemma 4 on the `--kv turbo4v2` path — long-context KV compression on sinks-using models without the path-A-only constraint. Path-B-only optimisation; path A remains the speed default for sinks models. Estimated decode tok/s on path B: 30–60% of path A (compressed-attention sub-phases). Worth it for the user segment that explicitly needs ≥16K context KV compression. **Also fixes one stuck Tier-1 prerequisite for spec 039:** spec 039's L>1 dispatcher branch on hydrated compressed caches currently checks `sinks == nil` and falls through to the raw path on sinks-using models — once this lands, spec 039 covers GPT-OSS-20B too. | 🚧 Four-repo PR chain OPEN: [mlx#16](https://github.com/ekryski/mlx/pull/16) (kernel + C++ primitive — pass2 folds per-head sink logit into cross-block softmax), [mlx-c#8](https://github.com/ekryski/mlx-c/pull/8) (C ABI + 5 missing decls), [mlx-swift#18](https://github.com/ekryski/mlx-swift/pull/18) (Swift wrapper + submodule bumps), [mlx-swift-lm#99](https://github.com/ekryski/mlx-swift-lm/pull/99) (path B CLI surface + sinks routing). Umbrella: [#130](https://github.com/ekryski/mlx-swift-lm/issues/130). **Promoted from Tier 4 (2026-05-12)** to close the "all model families at feature parity" gap alongside rows 6a + 6b. |
