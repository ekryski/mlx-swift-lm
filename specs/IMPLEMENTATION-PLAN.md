...
|---|---|---|---|
| 5 | ✅ **020 — state-replay rollback at the cache layer** (SHIPPED 2026-05-11) | Generalised the GDN rollback primitive so any speculative decoder works on hybrid SSM/Mamba models. Phases 1 + 2 + 3 consolidated and shipped via [PR #143](https://github.com/ekryski/mlx-swift-lm/pull/143) + cross-repo native kernel chain ([mlx#26](https://github.com/ekryski/mlx/pull/26), [mlx-c#14](https://github.com/ekryski/mlx-c/pull/14), [mlx-swift#25](https://github.com/ekryski/mlx-swift/pull/25)). `StateReplayCache` protocol + `SSMStateCache` conformance + native `gated_delta_step_record` / `state_replay` Metal kernels + n-gram iterator wiring. Mamba (Nemotron-H, Jamba) cleanly opts out via per-instance `canStateReplay = false` — future work to land their kernel variant tracked in spec 020's §"Mamba / Mamba 2 follow-up (post-MVP)". | Unlocked n-gram speculative decoding on Qwen 3.5 / 3.6 GDN hybrids. Measured wins on M1 Max 64GB: Qwen3.5-35B-A3B-4bit D=12 adapt+strict **1.64× baseline** (92% accept), D=4 **1.37×** (83% accept). Mamba families fall back to vanilla `TokenIterator` until kernel variant lands. |
| 6 | ✅ **017 — prefix KV cache** (READY-FOR-MERGE 2026-05-12) | Snapshot target KV state at request end keyed on stable prefix; hydrate on next request with the same prefix. **All five phases consolidated in [PR #144](https://github.com/ekryski/mlx-swift-lm/pull/144)**: Phase 1 (`PrefixKVCache` + `PrefixKey` + LRU + stats), Phase 1B (per-class `serialise()`/`hydrate(from:)` + `generate()` wiring with stream wrapper), Phase 2 (`LastAssistantOpenerPolicy` for Qwen/Gemma 4/GPT-OSS), Phase 3 (hybrid GDN+attention via spec 020 state-replay — `SSMStateCache` snapshots), Phase 4 (opt-in disk persistence at `~/.cache/mlx-swift-lm/prefix/`). Cross-session sharing via `BlockAllocator.retain()` tracked in [#133](https://github.com/ekryski/mlx-swift-lm/issues/133), separate future spec. | **Measured 2026-05-12 (M1 Max 64GB, --kv none):** Qwen3.5-0.8B ~3.0×, Gemma 4 E2B ~2.4×, **Qwen3.5-35B-A3B ~10.9×** (2861ms→262ms), Gemma 4 26B-A4B ~1.5×, Gemma 4 31B ~1.8×. GPT-OSS-20B: no benefit (window=128 sliding-window wrap, documented limitation). |
...
| 10a | **030 — Native MTP / EAGLE-3 draft heads** (promoted 2026-05-09 from Tier 4) | Variant A: stop stripping `mtp.*` at sanitize on DeepSeek-V4 / Qwen3.5 / Qwen3-Next / MiMo / GLM-4-MoE; ship `MTPSelfSpeculativeTokenIterator` + custom converter (`scripts/mtp_convert.py`). Variant B: companion EAGLE-style assistant draft + `AssistantDraftRegistry`. **First in the post-Tier-1 priority order** — largest pure-decode win, lossless, compounds multiplicatively with every other post-hoc spec below. Variant A on hybrid Qwen previously depended on Tier 1 #5 (spec 020) — **now unblocked** since spec 020 shipped 2026-05-11. Variant B + Variant A on DeepSeek-V4 also ship independently. | **1.4–2.2× decode** depending on family; assistant-draft path on Gemma 4 is decoder-agnostic (~2× at temp 0) | Universal — every supported model family |
...
|---|---|---|---|---|
| 11 | **015 phases 1–3** — DFlash on GPU | Port DFlash's Python reference (`bstnxbt/dflash-mlx engine-v2`) to MLX-Swift. Phase 1 (protocol surface + iterator scaffold) landed in [PR #141](https://github.com/ekryski/mlx-swift-lm/pull/141); phase 2 (real draft model from `z-lab/Qwen3.5-*-DFlash`) + phase 3 (hybrid GDN state-replay rollback — **prerequisite now landed**, refactor onto the `StateReplayCache` protocol from spec 020) need to land. | **2.4–4.4× on Qwen 3.5/3.6** | Standalone win once draft-model availability lands. Spec 020 (Tier 1 #5) is no longer a blocker (shipped 2026-05-11) — phase 3 now just refactors onto the protocol. |
