...
// Based on the Gemma3 VLM wrapper pattern + Gemma4 text decoder from MLXLLM
...
private class Gemma4TextAttention: Module {
    ...
    init(_ config: Gemma4TextConfiguration, layerIdx: Int) {
        ...
        if isSliding {
            // Sliding layers: standard RoPE with theta=10000, full dimensions
    ...
    /// Forward pass with optional KV sharing.
    /// - Parameters:
    ///   - sharedKV: Pre-computed (keys, values) from a donor layer. When provided,
    ///     this layer skips its own K/V projection and uses these instead.
    ///   - sharedOffset: The RoPE offset from the donor layer's cache.
    /// - Returns: Tuple of (output, keys, values) — keys/values are returned so they
    ///   can be shared with subsequent layers.
    func callAsFunction(
        _ x: MLXArray,
        ...
        if let (sharedKeys, sharedValues) = sharedKV {
            // KV sharing: reuse donor layer's K/V (already RoPE'd and normalized)
            let offset = sharedOffset ?? cache?.offset ?? 0
            queries = rope(queries, offset: offset)

            // Shared layers still need SDPA but skip cache update (donor owns the cache)
            ...
            return (oProj(output), sharedKeys, sharedValues)
        }
        ...
        // Store post-RoPE keys/values for sharing with subsequent layers
        let keysForSharing = keys
        let valuesForSharing = values
        ...
        return (oProj(output), keysForSharing, valuesForSharing)
    }
}
...
private class Gemma4TextTransformerBlock: Module {
    ...
    init(_ config: Gemma4TextConfiguration, layerIdx: Int) {
        ...
        // layer_scalar: initialized to [1.0], overwritten by checkpoint if present
    ...
    /// - Returns: Tuple of (output, keys, values) for KV sharing with later layers.
    func callAsFunction(
        _ x: MLXArray,
        mask: MLXFast.ScaledDotProductAttentionMaskMode,
        cache: KVCache? = nil,
        ...
        let (attnOut, keys, values) = selfAttention(
...
private class Gemma4TextModelInner: Module {
    ...
    func callAsFunction(
        ...
        // KV sharing: store intermediate K/V for donor layers
        var intermediateKVs: [(MLXArray, MLXArray)?] = Array(repeating: nil, count: layers.count)
        var intermediateOffsets: [Int?] = Array(repeating: nil, count: layers.count)
        ...
        for (i, layer) in layers.enumerated() {
            ...
            // KV sharing: if this layer reuses a donor's KV, pass it through
            let donorIdx = previousKVs[i]
            let isShared = donorIdx != i
            let sharedKV = isShared ? intermediateKVs[donorIdx] : nil
            let sharedOffset = isShared ? intermediateOffsets[donorIdx] : nil

            let (out, keys, values) = layer(
                h, mask: maskMode,
                cache: isShared ? nil : cache[i],  // shared layers don't own a cache
                perLayerInput: pli,
                sharedKV: sharedKV,
            ...
            intermediateKVs[i] = (keys, values)
            intermediateOffsets[i] = cache[i]?.offset
        }
...
private class Gemma4LanguageModel: Module, KVCacheDimensionProvider {
    ...
    func sanitize(
        ...
        // Handle weight tying
        if config.tieWordEmbeddings && processedWeights["language_model.lm_head.weight"] == nil {
            ["weight", "scales", "biases"].forEach { suffix in
                if let embedWeight = processedWeights[
                    "language_model.model.embed_tokens.\(suffix)"]
                {
                    processedWeights["language_model.lm_head.\(suffix)"] = embedWeight
                }
            }
        }
...
// MARK: - Vision Model Components (SigLIP-based, same as Gemma3)
...
private func gemma4MaskedScatter(
    ...
    guard scaledImageFeaturesFlattened.shape[0] == imagePositions.shape[0] else {
        fatalError(
            """
            Critical error in gemma4MaskedScatter: Size mismatch between image features and positions.
...
public class Gemma4: Module, VLMModel, KVCacheDimensionProvider {
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        // Handle language model sanitization first (weight tying, vocab trimming, etc.)
