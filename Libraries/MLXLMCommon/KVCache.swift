...
public class RotatingKVCache: BaseKVCache, CustomDebugStringConvertible {
    ...
    /// eliminating the per-chunk `concatenated` transient.
    ///
    ...
    ///   default — this is the path the iterators ship by default.
    ///
    /// **Why this is opt-in.** PR #138 measured wiring `reserve(prompt +
    /// maxTokens)` on by default in `TokenIterator` / `BatchTokenIterator`
    /// against Qwen3.5-9B B=2 ctx=32k: decode regressed ~43% (18.6 vs 26.5
    /// tok/s) with no peak-memory benefit. The default-on path made the
    /// common bench case slower and didn't help where peak actually lives
    /// (SDPA / MLP / quant-weight-dequant transients during prefill).
    /// The API stays available for callers that explicitly want the
    /// allocation shape — most useful when `maxKVSize` is set generously
    /// (e.g. a model's full context window) but the actual workload only
    /// uses a fraction.
