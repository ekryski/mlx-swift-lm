...
BRIDGE_DYLIB := $(PROJECT_ROOT)/Sources/NativePrefillBridge/libprefill_bridge_v2.dylib

# Output artifacts
METALLIB       := $(RELEASE_DIR)/mlx.metallib
BRIDGE_RELEASE := $(RELEASE_DIR)/libprefill_bridge_v2.dylib
...
$(STAMP_BRIDGE): $(BRIDGE_SRC) $(BRIDGE_HDR) | $(STAMP_DIR)
	@echo "==> Prefill bridge source changed — recompiling dylib..."
...
install-artifacts: $(STAMP_SPM) $(STAMP_METAL) $(STAMP_BRIDGE)
	...
	@# --- bridge dylib -> release dir ---
	@if [ -f "$(BRIDGE_DYLIB)" ]; then \
		cp "$(BRIDGE_DYLIB)" "$(RELEASE_DIR)/libprefill_bridge_v2.dylib"; \
		echo "  copied libprefill_bridge_v2.dylib -> $(RELEASE_DIR)/"; \
	fi
	@# --- bridge dylib -> test bundle ---
	@if [ -f "$(BRIDGE_DYLIB)" ] && [ -d "$(XCTEST_DIR)" ]; then \
		cp "$(BRIDGE_DYLIB)" "$(XCTEST_DIR)/libprefill_bridge_v2.dylib"; \
		echo "  copied libprefill_bridge_v2.dylib -> test bundle"; \
	fi
...
.PHONY: clean-bridge
clean-bridge:
	rm -f "$(BRIDGE_DYLIB)" "$(BRIDGE_RELEASE)"
	rm -f "$(STAMP_BRIDGE)"
	@echo "Bridge cache cleared. Next 'make' will recompile dylib."
...
status:
	...
	@[ -f "$(BRIDGE_DYLIB)" ] \
		&& echo "  libprefill_bridge_v2.dylib (source): OK" \
		|| echo "  libprefill_bridge_v2.dylib (source): MISSING"
	@[ -f "$(BRIDGE_RELEASE)" ] \
		&& echo "  libprefill_bridge_v2.dylib (release):OK" \
		|| echo "  libprefill_bridge_v2.dylib (release):MISSING"
	@echo ""
	@echo "SPM Cmlx cache:"
