...
| `gpt-oss-20b` + `--kv turbo*` | [#171](https://github.com/ekryski/mlx-swift-lm/issues/171) — non-Harmony / degenerate output on GPT-OSS-20B with TurboQuant | **Mitigated** by spec 041 phase 1.1 follow-up: `GPTOSSModel.newCache(...)` enables `useBias: true` on full-attention `TurboQuantizedKVCache`s and routes sliding-window layers to raw-FP16 `StandardKVCache`. Per-vector DC-bias correction recovers the structured mean lost by the zero-mean Lloyd-Max codebook (K/V projections have `bias=True` → structured DC offset). Override via `TURBO_BIAS=0` env var for A/B testing. Coherence verified at 1k + 8k summarization across `turbo4`, `turbo4v2`, `turbo8v4`. |
