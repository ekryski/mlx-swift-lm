...
| `AffineQuantizedKVCache.sdpaStrategy` | `.auto` | SDPA path selector (spec 041 phases 1+2+4). **`.auto`**: flash for L>1 (prefill) — no `[B, H, L, T]` score-tensor materialisation; discrete for L=1 (decode) — small score matrix, avoids per-step K/V dequant. **`.flash`**: force flash everywhere; wins on small-model long-context shapes where score materialisation dominates. **`.discrete`**: force discrete everywhere; regression-check / legacy materialise-scores path. Set via the `sdpaStrategy:` constructor parameter: `AffineQuantizedKVCache(bits: 4, sdpaStrategy: .flash)`. Env var `MLX_AFFINE_SDPA={auto,flash,discrete}` overrides this globally when set. |
