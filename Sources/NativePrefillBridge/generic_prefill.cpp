...
static array generic_attention(
    ...
    // Log counter — only log first call for the real (127-tok) forward
    // Skip warmup (first 24 calls) by checking S > 4
    static int attn_log_count = 0;
    bool L = (S > 4 && attn_log_count == 0);

    #define ATTN_LOG(label, tensor) if (L) { \
        auto _f = astype(reshape(tensor, {-1}), float32); eval({_f}); \
        auto* _v = static_cast<float*>(const_cast<allocator::Buffer&>(_f.buffer()).raw_ptr()); \
        fprintf(stderr, "[attn] %s: shape=[%d,%d,%d,%d] dtype=%d abssum=%.4f first2=[%.8f,%.8f]\n", \
            label, tensor.shape(0), tensor.ndim()>1?tensor.shape(1):0, \
            tensor.ndim()>2?tensor.shape(2):0, tensor.ndim()>3?tensor.shape(3):0, \
            (int)tensor.dtype().val(), safe_abssum(tensor), _v[0], _v[1]); \
    }

    if (L) fprintf(stderr, "[attn] === L0 attention step-by-step (S=%d) ===\n", S);
    ATTN_LOG("1-input x", x);

    auto q = q_proj(x);
    auto k = k_proj(x);
    auto v = v_proj(x);
    ATTN_LOG("2-q_proj out", q);
    ATTN_LOG("2-k_proj out", k);
    ATTN_LOG("2-v_proj out", v);
    ...
    ATTN_LOG("3-q reshaped", q);
    ATTN_LOG("3-v reshaped", v);
    ...
    if (yarn_mscale != 1.0f) {
        ...
        if (L) fprintf(stderr, "[attn] 4-yarn_mscale=%.6f\n", yarn_mscale);
    }

    ATTN_LOG("5-pre-RoPE q", q);
    ATTN_LOG("5-pre-RoPE k", k);
    ...
    if (rope_freqs && rope_freqs->ndim() == 1) {
        ...
        if (L) fprintf(stderr, "[attn] 6-RoPE: YaRN freqs, offset=%d\n", rope_offset);
    } else {
        ...
        if (L) fprintf(stderr, "[attn] 6-RoPE: vanilla theta=%.0f rot=%d offset=%d\n", rope_theta, actual_rotary, rope_offset);
    }
    ATTN_LOG("7-post-RoPE q", q);
    ATTN_LOG("7-post-RoPE k", k);

    auto kv_dtype = k_proj.scales ? k_proj.scales->dtype() : float16;
    k = astype(k, kv_dtype);
    v = astype(v, kv_dtype);
    if (L) fprintf(stderr, "[attn] 8-cast KV to dtype=%d\n", (int)kv_dtype.val());

    cache.append(k, v, S);
    if (L && cache.keys) {
        auto& ck = *cache.keys; auto& cv = *cache.values;
        auto _f = astype(reshape(ck, {-1}), float32); eval({_f});
        auto* _v = static_cast<float*>(const_cast<allocator::Buffer&>(_f.buffer()).raw_ptr());
        fprintf(stderr, "[attn] 9-cache.keys: shape=[%d,%d,%d,%d] dtype=%d abssum=%.4f first2=[%.8f,%.8f]\n",
            ck.shape(0), ck.shape(1), ck.shape(2), ck.shape(3), (int)ck.dtype().val(), safe_abssum(ck), _v[0], _v[1]);
        auto _f2 = astype(reshape(cv, {-1}), float32); eval({_f2});
        auto* _v2 = static_cast<float*>(const_cast<allocator::Buffer&>(_f2.buffer()).raw_ptr());
        fprintf(stderr, "[attn] 9-cache.vals: shape=[%d,%d,%d,%d] dtype=%d abssum=%.4f first2=[%.8f,%.8f]\n",
            cv.shape(0), cv.shape(1), cv.shape(2), cv.shape(3), (int)cv.dtype().val(), safe_abssum(cv), _v2[0], _v2[1]);
    }
    if (L) fprintf(stderr, "[attn] 9-cache offset=%d\n", cache.offset);

    float scale = custom_scale > 0.0f ? custom_scale
                  : 1.0f / std::sqrt(static_cast<float>(head_dim));
    if (L) fprintf(stderr, "[attn] 10-SDPA scale=%.8f\n", scale);

    auto attn = fast::scaled_dot_product_attention(
        q, *cache.keys, *cache.values, scale, "causal", std::nullopt, sinks);
    ATTN_LOG("11-SDPA out", attn);

    attn = reshape(transpose(attn, {0, 2, 1, 3}), {B, S, num_heads * head_dim});
    ATTN_LOG("12-attn reshaped", attn);

    auto out = o_proj(attn);
    ATTN_LOG("13-o_proj out", out);

    if (L) {
        fprintf(stderr, "[attn] === end L0 ===\n");
        attn_log_count++;
    }
...
struct GenericModel {
    ...
    static void dbg(const char* label, const array& t) {
        auto f = astype(reshape(t, {-1}), float32);
        eval({f});
        auto* v = static_cast<float*>(const_cast<allocator::Buffer&>(f.buffer()).raw_ptr());
        fprintf(stderr, "[gp] FWD %s: shape=[%d,%d,%d] abssum=%.4f first2=[%.6f,%.6f]\n",
            label, t.shape(0), t.shape(1), t.ndim()>2?t.shape(2):0, safe_abssum(t), v[0], v[1]);
    }
    ...
    array forward(const array& token_ids) {
        ...
        for (int c = 0; c < num_chunks; c++) {
            ...
            if (num_chunks > 1) {
                fprintf(stderr, "[gp] chunk %d/%d done (%d tokens)\n",
                    c + 1, num_chunks, chunk_len);
            }
        }
...
static void build_gptoss_layer(Layer& layer, int idx) {
    ...
    fprintf(stderr, "[gp] L%d rope_type='%s' yarn_factor=%.1f\n", idx, g_config.rope_type.c_str(), g_config.yarn_factor);
    if (g_config.rope_type == "yarn" && g_config.yarn_factor > 1.0f) {
        ...
        if (idx == 0) {
            fprintf(stderr, "[gp] YaRN: mscale=%.6f freqs ndim=%d size=%d first2=[%.4f,%.4f]\n",
                mscale, freqs.ndim(), (int)freqs.size(),
                safe_abssum(slice(freqs, {0}, {1})),
                safe_abssum(slice(freqs, {1}, {2})));
        }
    }
