...
actor ServerPromptCache {
    ...
    func fetch(tokens newTokens: [Int], model: any LanguageModel) -> ([KVCache], [Int], CacheStatus, UUID) {
        ...
        if bestIdx >= 0 && bestPrefix > 0 {
            ...
            // use in-place. If it diverges (different suffix), deep copy so the
            // original stays intact for future reuse.
...
/// queue until a slot frees up (FIFO). This limits concurrent model access to N slots.
///
...
final class SimpleHTTPServer {
    ...
    func start() throws {
        ...
        log("Endpoints: GET /v1/models, POST /v1/chat/completions, POST /v1/completions, GET /health, GET /metrics, GET /slots")
    ...
    func handleClient(_ fd: Int32) async {
        ...
        do {
            ...
            // Parse Content-Length and Origin
            var contentLength = 0
            for line in lines {
    ...
    func handleChat(fd: Int32, request: ChatRequest, corsOrigin: String = "*") async {
        ...
        do {
            ...
            // For models that don't support "tool" role, convert tool results
            // to user messages and tool_calls to assistant messages.
            ...
            // Keepalive timer: sends SSE chunks every 2s during prefill to prevent
            // client ReadTimeout. Uses GCD because generate() blocks the cooperative
            // thread pool during GPU inference.
            ...
            if isStreaming {
                ...
                do {
                    ...
                    for try await generation in try generate(
                        ...
                        case .chunk(let text):
                            ...
                            // After tool calls, suppress trailing XML tags
                            if hadToolCall {
                            ...
                            // Check for potential tool call in un-emitted portion
                            ...
                            if unemitted.contains("<tool_call>") || unemitted.contains("<function=") || unemitted.contains("<minimax:tool_call>") || unemitted.contains("<invoke name=") {
                                ...
                                // Not a known tag — emit everything
                                writeSSE(fd: fd, requestId: requestId, role: nil, content: unemitted, finishReason: nil, requestModel: reqModel)
                                emittedUpTo = fullText.count
                            } else {
                ...
                // Strip <think>...</think> blocks, preserving as reasoning_content
                ...
                // Parse tool calls from accumulated text (XML formats)
    ...
    /// Parse ALL `<invoke>` blocks from MiniMax tool call XML (supports multiple tools per response).
    func parseAllMiniMaxToolCalls(_ text: String) -> [(name: String, arguments: String)] {
        ...
        if results.isEmpty && text.contains("<parameter name=") && !text.contains("<invoke name=") {
            ...
            if !args.isEmpty {
                // TODO: Improve tool name inference — consider using request context
                let toolName: String
