...
| `MLX_SMART_MEMORY` | `0`, `1`, or unset | `1` (on) | Smart wired-memory sizing. When on, the bench computes a per-run wired-memory ticket from the loaded model's actual `kvHeads × headDim`, the configured KV cache scheme, the max context, and the batch size. Set to `0` to fall back to `GPU.maxRecommendedWorkingSetBytes()` (no estimation). Overridden when `MLX_MEMORY_LIMIT` is set. |
| `MLX_MEMORY_LIMIT` | bytes (e.g. `34359738368`) or human-friendly (`32g`, `32GB`, `512m`, `4k`, `1.5g`) | unset | Explicit wired-memory limit. Bypasses both the smart estimate and `MLX_SMART_MEMORY`. Clamped to `GPU.maxRecommendedWorkingSetBytes()` when available. Use this when running at large batch / long context where the estimator over- or under-shoots. Case-insensitive; rejects zero and negative values. |
