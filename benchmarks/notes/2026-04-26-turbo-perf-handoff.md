...
| H12 | Buffer-pointer collision in pooled allocator (hazard tracker fooled by reuse) | ⚠️ partially confirmed but not load-bearing — trace clearly shows `prev_outputs_ = std::move(...)` REPLACE-not-UNION causes pass2-output ptrs to fall out of the tracker, but switching pass2 to bf16 output dodges the issue entirely. Worth investigating as a generic upstream MLX bug; see "Session 3 H12 deep-dive" below. |
